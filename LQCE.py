...
def load(args):
    ...
    loaded_fee = load_model(fee_importer_params, log_level = logging.INFO)
    ...
    net_list_0, net_list_1, net_list_2, inp_list_0, inp_list_1, inp_list_2, fmap1, fmap2 = loaded_fee.execute(fee_inputs, log_level = logging.INFO)
...
def quantize_model(loaded_model, calibrated_data, model_name, int16 = False):

    if int16:
        quant_configs = int16_quantization
    else:
...
def quantized_execution(args, fee_quantized, refiner_quantized, fast_mode = False):
    ...
    for itr in tqdm(range(args.valid_iters)):
        ...
        corr = corr_fn(coords1).detach().cpu().numpy()
        refiner_inputs['corr'] = np.transpose(corr, (0, 2, 3, 1))
        
        flow = coords1 - coords0
        flow = flow.detach().cpu().numpy()
        refiner_inputs['flow'] = np.transpose(flow, (0, 2, 3, 1))

        # executing quantized refiner
        refiner_inputs['net_list_0.1'], refiner_inputs['net_list_1.1'], refiner_inputs['net_list_2.1'], up_mask, delta_flow = refiner_quantized.execute(refiner_inputs, fast_mode = fast_mode, log_level = logging.INFO)
        
        delta_flow = torch.from_numpy(np.transpose(delta_flow, (0, 3, 1, 2)))
        delta_flow[:, 1] = 0.0
        ...
        if up_mask is None:
            ...
            up_mask = torch.from_numpy(np.transpose(up_mask, (0, 3, 1, 2)))
            flow_up = upsample_flow(args, coords1 - coords0, up_mask) 
        
        flow_up = flow_up[:, :1]
    
    plt.imsave(f"output_disparity/sima_only_output.png", -flow_up.detach().numpy().squeeze(), cmap = 'jet')
