...
class BasicMultiUpdateBlock(nn.Module):
    ...
    def forward(self, net_list_0, net_list_1, net_list_2, inp, corr = None, flow = None, iter08 = True, iter16 = True, iter32 = True, update = True):

        if iter32:
            net_list_2 = self.gru32(net_list_2, *(inp[2]), pool2x(net_list_1))
            #net[2] = self.gru32(net[2], *(inp[2]), pool2x(net[1]))
        if iter16:
            if self.args.n_gru_layers > 2:
                net_list_1 = self.gru16(net_list_1, *(inp[1]), pool2x(net_list_0), interp(net_list_2, net_list_1))
                #net[1] = self.gru16(net[1], *(inp[1]), pool2x(net[0]), interp(net[2], net[1]))
            else:
                net_list_1 = self.gru16(net_list_1, *(inp[1]), pool2x(net_list_0))
                #net[1] = self.gru16(net[1], *(inp[1]), pool2x(net[0]))
        if iter08:
            motion_features = self.encoder(flow, corr)
            if self.args.n_gru_layers > 1:
                net_list_0 = self.gru08(net_list_0, *(inp[0]), motion_features, interp(net_list_1, net_list_0))
                #net[0] = self.gru08(net[0], *(inp[0]), motion_features, interp(net[1], net[0]))
            else:
                net_list_0 = self.gru08(net_list_0, *(inp[0]), motion_features)
                #net[0] = self.gru08(net[0], *(inp[0]), motion_features)

        if not update:
            return net_list_0, net_list_1, net_list_2

        #delta_flow = self.flow_head(net[0])
        delta_flow = self.flow_head(net_list_0)

        # scale mask to balence gradients
        #mask = torch.multiply(.25, self.mask(net[0]))
        mask = .25 * self.mask(net_list_0)
        #mask = torch.multiply(.25, self.mask(net_list_0))

        #return net, mask, delta_flow
        return net_list_0, net_list_1, net_list_2, mask, delta_flow
